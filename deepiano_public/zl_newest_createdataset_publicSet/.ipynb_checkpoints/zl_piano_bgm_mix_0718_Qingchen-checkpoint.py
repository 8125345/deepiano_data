...
tf.app.flags.DEFINE_string('input_dir', '/deepiano_data/yuxiaofei/work/data_0718/piano/split_trans_noise_data_Qingchen_SC55', '')
...
tf.app.flags.DEFINE_string('output_dir', '/deepiano_data/yuxiaofei/work/data_0718/mix_changpu_Qingchen_SC55', '')
...
# def generate_wav_files_set(input_dir):
...
def add_pre_recorded_noise(wav_filename, mid_filename, output_dir, noise_dir):
    ...
    concat_num = math.ceil(input_duration/noise_duration)
    if concat_num > 1:
      noise_list = []
      for i in range(max(concat_num,2)):
        noise_list.append(noise_file)
      cbn = sox.Combiner()
      tmp_noise_file = 'tmp_noise.wav'
      cbn.build(noise_list, tmp_noise_file, 'concatenate')
    else:
      tmp_noise_file = noise_file

    trim_command = 'sox {noise_file} {output_noise_file} trim {start} {input_duration}'.format(**{
        'noise_file': tmp_noise_file,
