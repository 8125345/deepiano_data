...
tf.app.flags.DEFINE_enum('dataset', 'maestro-v3.0.0', ['ai-tagging', 'high-note', 'single-note', 'maestro-v3.0.0', 'Qingchen_xml', 'Qingchen_xml_SC55', 'Peilian_xml', 'Peilian_xml_SC55'], 'which dataset will be used')
...
将音频切分成最长20s的多个音频片段
"""
def wav_data_mid_to_serialize(wav_data, ns, dst_dir, min_length=5, max_length=20, pre_audio_transform=False, hparams=None):
    try:
...
def generate_maestro_dataset(dataset, pre_audio_transform=False, hparams=None):
    ...
    if pre_audio_transform:
        if FLAGS.transform_noise_enable:
            output_dir = os.path.join(FLAGS.output_dir, 'noise_trans/maestro-v3.0.0_{}'.format(time.strftime("%Y%m%d_%H%M%S", time.localtime())))
        else:
            output_dir = os.path.join(FLAGS.output_dir, 'trans/maestro-v3.0.0_{}'.format(time.strftime("%Y%m%d_%H%M%S", time.localtime())))
    else:
        output_dir = os.path.join(FLAGS.output_dir, 'std/maestro-v3.0.0_{}'.format(time.strftime("%Y%m%d_%H%M%S", time.localtime())))
    ...
    for _, midi_filename, audio_filename in dataset:
        ...
        if os.path.isfile(fixed_midi_path) and os.path.isfile(fixed_audio_path):
            try:
                ...
                wav_data_mid_to_serialize(wav_data, ns, tmp_output_dir, min_length=FLAGS.min_length, max_length=FLAGS.max_length, pre_audio_transform=FLAGS.transform_audio, hparams=hparams) 
