...
def write_json(train_file_set, test_file_set, validation_set, seri_files, json_dir, DS):
    train_dirs = []
    test_dirs = []
    validation_dirs = []
    ...
    if validation_dirs:
        ...
        with open(os.path.join(json_dir, f"{DS}_validation.json"), "w+") as json_file:
            json.dump(validation_json_data, json_file, ensure_ascii=False)



if __name__ == '__main__':
    serialize_dir = '/deepiano_data/zhaoliang/public_data/serialize_mix_public'
    json_dir = '/deepiano_data/zhaoliang/public_data/json_mix_public'
    ...
    for DS in dataset:
        ...
        write_json(train_file_set, test_file_set, validation_set, serialize_files, json_dir, DS)
        print(f'{DS}数据集处理完成............')
