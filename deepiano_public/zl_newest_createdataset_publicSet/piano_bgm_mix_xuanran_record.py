...
tf.app.flags.DEFINE_string('input_dir',
                           '/deepiano_data/yuxiaofei/work/data_0718/piano/split_trans_noise_data/noise_trans/ai-tagging_20220721_183306',
                           '')
...
def process(wav_mid_pairs):
    ...
    for wav_file, mid_file in wav_mid_pairs:
        ...
        x = wav_file.split('/')
        output_dir = FLAGS.output_dir + '/' + x[-6] + '/' + x[-5] + '/' + x[-4] + '/' + x[-3] + '/' + x[-2]
        print('output_dir: \n', output_dir)
