...
root_dir = '/deepiano_data/yuxiaofei/work/data_0718/serialize_changpu_delay_Qingchen_SC55_metronome' #'./data/serialize' # 
iter_files(root_dir)
...
# high-note
csv_dir = '/deepiano_data/dataset/Qingchen_xml_SC55/'
csv_file = '/deepiano_data/dataset/Qingchen_xml_SC55/split.csv'
train_file_set = []
test_file_set = []
with open(csv_file) as f:
    items = csv.reader(f)
...
train_json_data = dict(zip(range(len(train_dirs)), train_dirs))
with open(os.path.join(root_dir, "Qingchen_xml_SC55_train.json"), "w+") as json_file:
    json.dump(train_json_data, json_file, ensure_ascii=False)

test_json_data = dict(zip(range(len(test_dirs)), test_dirs))
with open(os.path.join(root_dir, "Qingchen_xml_SC55_test.json"), "w+") as json_file:
    json.dump(test_json_data, json_file, ensure_ascii=False)
